...
<section class="blog-posts commonPadding-120 ">
    <div class="container-ctn">
        <div class="blog-posts-latest d-flex flex-wrap justify-content-between">
            @foreach($firstTwoBlogs as $twoBlog)
                <a class="blog-posts-card" href="{{ route('blogDetail', ['short_url' => $twoBlog->short_url]) }}">
                    <picture>
                        <img loading="lazy" src="{{asset('uploads/blog/image/'. $twoBlog->image)}}" width="939" height="430" alt="" class="img-fluid" alt="{{$twoBlog->image_meta_tag}}">
                    </picture>
                    <h3>{{$twoBlog->title}}</h3>
                    {{ Str::limit(strip_tags($twoBlog->description), 150, '...') }}
                    <div class="blog-post-meta d-flex flex-wrap justify-content-between">
                        <div class="d-flex flex-wrap" >
                                <div class="author-image">
                                    @if($twoBlog->author_image)
                                        <img loading="lazy"  src="{{asset('uploads/blog/image/' . $twoBlog->author_image)}}" alt="{{$twoBlog->author_name}}">{{$twoBlog->author_name}}
                                    @else
                                        <img loading="lazy" src="{{asset('uploads/blog/image/av.png')}}"> Admin
                                    @endif
                                </div>
                                <span>{{ \Carbon\Carbon::parse($twoBlog->posted_date)->format('M d, Y') }}</span>
                        </div>
        ...
        @include('web.partials._blogs_list', ['blogs' => $blogs])
