...
<section class="service-detail commonPadding-120 pb-0">
    <div class="container-ctn">
           ...
           <picture>
                @if(!empty($service->webp_image))
                    <img src="{{ asset('uploads/service/webp/'.$service->webp_image) }}"
                        width="1684"
                        height="601"
                ...
                <article>
                    {!! $service->home_description !!}
                        ...
                        @if($service->brochure || $siteInformation->service_brochure)
                            ...
                            target="_blank">
                                <button class=" btn theme-btn theme-bg" role="button">Download Brochure</button>
                    ...
                    <div class="d-flex flex-wrap article-picture ">
                        @if(isset($service->activeGalleries) && !empty($service->activeGalleries))
                            @foreach($service->activeGalleries as $image)
                            <picture>
                                <img src="{{asset('uploads/service/gallery/'.$image->service_id.'/'.$image->image)}}" width="478" height="462" alt="">
                            </picture>
                            @endforeach
                        @endif
                    </div>
                    <h3>{{$service->second_title}} </h3>
                ...
                <aside>
                    ...
                    <ul>
                        @foreach($otherServices as $service)
                            <li>
                                <a href="{{ route('serviceDetail', ['short_url' => $service->short_url]) }}">{{ $service->title }}</a>
                            </li>
                        @endforeach
                    </ul>
        ...
        <div class="enquiry-wrapper">
            ...
            <form action="#" id="serviceEnquiryForm">
                ...
                <div class="button-wrap d-flex flex-wrap ">
                    <button type="submit" class="btn service-btn btn-submit" data-url="/service/enquiry" id="contact_form_btn">SUBMIT</button>
